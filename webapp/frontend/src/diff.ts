...
function lcsDiff(oldTokens: string[], newTokens: string[]): DiffPart[] {
  ...
  while (i < n) parts.push({ value: oldTokens[i++], removed: true });
  while (j < m) parts.push({ value: newTokens[j++], added: true });
  return parts;
}
...
export function computeStats(parts: DiffPart[]): DiffStats {
  const s: DiffStats = { added: 0, removed: 0, unchanged: 0, charsAdded: 0, charsRemoved: 0 };
  for (const p of parts) {
    if (p.added) { s.added++; s.charsAdded += p.value.length; }
    else if (p.removed) { s.removed++; s.charsRemoved += p.value.length; }
    else s.unchanged++;
  }
  return s;
}

export function filterParts(parts: DiffPart[], filter: DiffFilter): DiffPart[] {
