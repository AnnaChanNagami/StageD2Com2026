...
export function initCorrections(): void {
  const jsonEl = document.getElementById("corrChartData") as HTMLScriptElement | null;
  if (!jsonEl?.textContent) return;
  const d: CorrChartData = JSON.parse(jsonEl.textContent);
    ...
    new Chart(document.getElementById("error-chart"), {
      ...
      options: {
        ...
        scales: {
          ...
          y: { ticks: { color: "#64748b", callback: (v: number) => v + "%" }, grid: { color: "rgba(148,163,184,0.1)" }, beginAtZero: true },
        },
      },
    });
