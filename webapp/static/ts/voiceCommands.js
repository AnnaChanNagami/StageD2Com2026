...
function toggleMicIcon(listening) {
...
function executeCommand(cmd) {
    ...
    switch (cmd) {
        case 'lancer la transcription':
            ...
            const uploadBtn = document.querySelector('.drop-zone');
            if (uploadBtn)
                uploadBtn.click();
            break;
        case 'arrêter':
...
export function initVoiceCommands() {
    ...
    if (voiceBtn) {
        voiceBtn.addEventListener('click', () => {
            const modal = new window.bootstrap.Modal(document.getElementById('voiceCommandModal'));
            modal.show();
        });
    }
...
window.voiceCommands = { startVoiceRecognition, stopVoiceRecognition };
