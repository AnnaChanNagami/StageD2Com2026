...
import { computeStats, diffWords, diffChars, diffLines, escapeHtml, filterParts } from "./diff.js";
export function initJobDetail() {
    const jsonEl = document.getElementById("scribeJobData");
    if (!jsonEl || !jsonEl.textContent)
        return;
    const pageData = JSON.parse(jsonEl.textContent);
    ...
    const wcCanvas = document.getElementById("wcChart");
...
// --- Diff UI ---
function initDiff(originalFull, correctedFull) {
    let diffMode = "word";
    let diffFilter = "all";
    const computeDiff = () => {
        if (diffMode === "word")
            return diffWords(originalFull, correctedFull);
        if (diffMode === "char")
            return diffChars(originalFull, correctedFull);
        return diffLines(originalFull, correctedFull);
    };
    const renderParts = (parts, mode) => {
        let html = "";
    ...
    const applyDiff = () => {
        ...
        if (out)
            out.innerHTML = renderParts(display, diffMode);
