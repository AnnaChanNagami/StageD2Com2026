...
function legendOptions(position = "top") {
    return { legend: { position, labels: { color: LEGEND_DARK } } };
}
function canvasOr(parentId) {
    const el = document.getElementById(parentId);
    return el?.parentElement ?? null;
}
function emptyState(parentId, icon, message) {
...
export function initDashboard() {
    const jsonEl = document.getElementById("dashboardData");
    ...
    // Langues (donut)
    const lang = document.getElementById("langChart");
    if (lang) {
        new Chart(lang, {
            type: "doughnut",
            data: {
    ...
    const timeCompare = document.getElementById("timeCompareChart");
    if (timeCompare) {
    ...
    const wordsChars = document.getElementById("wordsCharsChart");
    if (wordsChars) {
    ...
    const corrChart = document.getElementById("correctionChart");
    if (corrChart) {
    ...
    const corrCompare = document.getElementById("corrCompareChart");
    if (corrCompare) {
        if (d.corrections.length > 0) {
    ...
    const errorType = document.getElementById("errorTypeChart");
    if (errorType) {
    ...
    const errorTrend = document.getElementById("errorTrendChart");
    if (errorTrend) {
